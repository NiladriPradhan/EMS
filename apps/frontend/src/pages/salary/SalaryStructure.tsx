...
const SalaryStructurePage = () => {
  ...
  const [deleteTarget, setDeleteTarget] = useState<SalaryStructure | null>(
    null,
  );
  ...
  const onSubmit = (values: FormValues) => {
    if (editTarget) {
      // ✅ Ensure ID is always a string (handles both string and number)
      const id = String(editTarget._id || editTarget.salary_structure_id!);
    ...
    <Box>
              ...
              : filteredData?.map((s, idx) => {
                  ...
                  return (
                    <TableRow key={s._id || s.salary_structure_id} hover>
                      ...
                      <TableCell>
                        {s.employee_id?.first_name
                          ? `${s.employee_id.first_name} ${s.employee_id.last_name}`
                          : s.employee_name || `EMP#${s.employee_id}`}
                      </TableCell>
      ...
      <Dialog open={open} onClose={handleClose} maxWidth="md" fullWidth>
        <form onSubmit={handleSubmit(onSubmit)}>
          <DialogTitle>
            {editTarget ? "Edit Salary Structure" : "Add Salary Structure"}
          </DialogTitle>
          <DialogContent sx={{ pt: 2 }}>
            ...
            >
              ...
              <Typography
                variant="subtitle2"
                sx={{ gridColumn: "1 / -1", color: "textSecondary" }}
              >
                Earnings
              </Typography>
      ...
      <Dialog open={!!deleteTarget} onClose={() => setDeleteTarget(null)}>
        ...
        <DialogContent>
          <Typography>Delete this structure? This cannot be undone.</Typography>
        </DialogContent>
        <DialogActions>
          ...
          <Button
            ...
            onClick={() => {
              if (deleteTarget) {
                ...
                const id = String(
                  deleteTarget._id || deleteTarget.salary_structure_id!,
                );
